...
var composeColorValue = (tween, progress, precision) => {
	...
	const r = round$1(clamp$1(mod(lerp$1(fn[0], tn[0], progress)), 0, 255), 0);
	const g = round$1(clamp$1(mod(lerp$1(fn[1], tn[1], progress)), 0, 255), 0);
	const b = round$1(clamp$1(mod(lerp$1(fn[2], tn[2], progress)), 0, 255), 0);
	const a = clamp$1(mod(round$1(lerp$1(fn[3], tn[3], progress), precision)), 0, 1);
...
var composeComplexValue = (tween, progress, precision) => {
	...
	for (let j = 0, l = tn.length; j < l; j++) {
		...
		const s = ts[j + 1];
		v += `${s ? n + s : n}`;
		if (hasComposition) tween._numbers[j] = n;
	}
	return v;
};
//#endregion
		...
		if (!_hasChildren) {
			...
			while (tween) {
				...
				if ((forcedRender || (tweenCurrentTime !== tweenChangeDuration || absoluteTime <= tweenAbsEndTime + (tweenNextRep ? tweenNextRep._delay : 0)) && (tweenCurrentTime !== 0 || absoluteTime >= tween._absoluteStartTime)) && (!tweenHasComposition || !tween._isOverridden && (!tween._isOverlapped || absoluteTime <= tweenAbsEndTime) && (!tweenNextRep || tweenNextRep._isOverridden || absoluteTime <= tweenNextRep._absoluteStartTime) && (!tweenPrevRep || tweenPrevRep._isOverridden || absoluteTime >= tweenPrevRep._absoluteStartTime + tweenPrevRep._changeDuration + tween._delay))) {
					...
					} else if (tweenValueType === valueTypes.COLOR) value = composeColorValue(tween, tweenProgress, tweenPrecision);
		...
		for (let targetIndex = 0; targetIndex < targetsLength; targetIndex++) {
			...
			for (let p in params) if (isKey(p)) {
				...
				for (let l = keyframes.length; tweenIndex < l; tweenIndex++) {
					...
					else if (vt === valueTypes.COLOR) tween._value = composeColorValue(tween, 1, -1);
		...
		if (!values.length) {
			if (autoGrid) {
				...
				if (hasPositions) {
					...
					if (minDist > 0 && minDist < Infinity) for (let index = 0, l = values.length; index < l; index++) values[index] = values[index] / minDist;
	...
	return (target, index, targets, prevTween) => {
		...
		if (!prevTween) {
			if (override === "") fillStartText = "";
				...
				fillStartText = "";
				for (let c = 0; c < startLength; c++) fillStartText += startingText[c] === " " ? " " : overrideCache[c];
			}
		}
		let lastValue = -1;
		let lastStep = -1;
