...
function GifImage({
  ...
  gif: { url: string; x: number; y: number; width: number; height: number };
  index: number;
  onDragEnd: (index: number, x: number, y: number) => void;
}) {
  const [image] = useImage(gif.url, "anonymous");
  return (
    <KonvaImage
      image={image}
      x={gif.x}
...
function Edit() {
  ...
  async function fetchGifs(e?: React.FormEvent) {
    e?.preventDefault();
    if (!gifSearch.trim()) return;
    try {
      const response = await fetch(
        `https://api.giphy.com/v1/gifs/search?api_key=${import.meta.env.VITE_GIPHY_API_KEY}&q=${encodeURIComponent(gifSearch)}&limit=8&rating=g`
      );
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Could not get GIFs.");
      const urls: string[] = data.data.map((item: any) => item.images.original.url);
      setGifResults(urls);
    } catch (err: any) {
      setError({ status: true, message: err.message || "Could not load GIFs." });
    }
  }

  function createGif(url: string) {
    const currGif = { url, x: 50, y: 50, width: 150, height: 150 };
    if (cardSide === "Front") {
      setFrontCard((prev) =>
        prev.map((card, index) =>
          index === cardNum - 1 ? { ...card, gif: [...card.gif, currGif] } : card
        )
      );
    } else {
      setBackCard((prev) =>
                ...
                <div className={styles.cardFront}>
                  ...
                  >
                    <Layer>
                      ...
                      {frontCard[cardNum - 1].gif.map((gif, gifIndex) => (
                        <GifImage
                          key={gifIndex}
                          gif={gif}
                          index={gifIndex}
                          onDragEnd={(index, x, y) => {
                            setFrontCard((prev) =>
                              prev.map((card, cardIndex) =>
                                cardIndex === cardNum - 1
                                  ? { ...card, gif: card.gif.map((tmp, i) => i === index ? { ...tmp, x, y } : tmp) }
                                  : card
                              )
                            );
                          }}
                ...
                <div className={styles.cardBack}>
                  ...
                  >
                    <Layer>
                      ...
                      {backCard[cardNum - 1].gif.map((gif, gifIndex) => (
                        <GifImage
                          key={gifIndex}
                          gif={gif}
                          index={gifIndex}
